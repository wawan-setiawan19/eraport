...
<h3><b style="color:#d81b60;">Madrasah Diniyah</b></h3>
    <table class="table table-bordered">
      ...
      <?php 
      $md=mysqli_query($con,"SELECT * FROM mapelmd_kelas left join mapelmd on (mapelmd.id_mapelmd=mapelmd_kelas.id_mapelmd) where c_kelas='$_GET[r]' order by nama_mapelmd asc "); $vr=1;
      while($hmd=mysqli_fetch_array($md)){
        $nilmd= mysqli_fetch_array(mysqli_query($con,"SELECT * FROM nilaimd where c_ta='$c_ta' and c_siswa='$_GET[q]' and c_kelas='$_GET[r]' and id_mapelmd='$hmd[id_mapelmd]' "));
          echo '
          <tr>
    ...
    <table class="table table-bordered">
      ...
      <?php 
      $md=mysqli_query($con,"SELECT * FROM mapeltq_kelas left join mapeltq on (mapeltq.id_mapeltq=mapeltq_kelas.id_mapeltq) where c_kelas='$_GET[r]' order by nama_mapeltq asc "); $vr=1;
      while($hmd=mysqli_fetch_array($md)){
        $niltq= mysqli_fetch_array(mysqli_query($con,"SELECT * FROM nilaitq where c_ta='$c_ta' and c_siswa='$_GET[q]' and c_kelas='$_GET[r]' and id_mapeltq='$hmd[id_mapeltq]' "));
          echo '
          <tr>
            <td class="text-center">'.$vr.'</td>
