...
//deskripsi rapot
    ...
    $at=date('Y-m-d');$des_peng = $_POST['des_peng']; $des_ket = $_POST['des_ket'];
    $lsis=mysqli_query($con,"SELECT * FROM siswa where c_kelas='$_POST[c_kelas]' order by nama asc "); $no=0; while($hlsis=mysqli_fetch_array($lsis)){
      //disini inputnya
        if($des_peng[$no]==''){}else{
          $cekada=mysqli_fetch_array(mysqli_query($con,"SELECT * FROM deskripsi_rapot where c_ta='$c_ta' and c_siswa='$hlsis[c_siswa]' and c_kelas='$_POST[c_kelas]' and c_mapel='$_POST[c_mapel]' "));
          if($cekada==NULL){
            $input=mysqli_query($con,"INSERT INTO deskripsi_rapot set c_ta='$c_ta',c_siswa='$hlsis[c_siswa]',c_kelas='$_POST[c_kelas]',c_mapel='$_POST[c_mapel]',des_peng='$des_peng[$no]',des_ket='$des_ket[$no]' ");
          }else{
             $edit=mysqli_query($con,"UPDATE deskripsi_rapot set c_ta='$c_ta',c_siswa='$hlsis[c_siswa]',c_kelas='$_POST[c_kelas]',c_mapel='$_POST[c_mapel]',des_peng='$des_peng[$no]',des_ket='$des_ket[$no]' WHERE c_deskripsi='$cekada[c_deskripsi]' ");
          }
        }
        $no++;
    }
    ...
    $c_kelas=$_GET['q']; $c_mapel=$_GET['r'];
    $setel=mysqli_query($con,"DELETE from deskripsi_rapot where c_ta='$c_ta' and c_kelas='$c_kelas' and c_mapel='$c_mapel' ");
    session_start();
    $_SESSION['pesan']='ulang';
