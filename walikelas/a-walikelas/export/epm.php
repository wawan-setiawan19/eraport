...
$mapel= mysqli_query($con,"SELECT * FROM mapel where c_mapel='$_GET[q]' "); foreach($mapel as $hmapel);
    ...
    foreach($siswa as $hsiswa){
      ...
      $objset->setCellValue('B'.$m,'   '.$hsiswa['nisn']);
      $objset->setCellValue('C'.$m,'   '.$hsiswa['nama']);
      if($hsiswa['nilaimid']==NULL){}else{
        if($hsiswa['nilaimid']>=$hmapel['sl']){
    ...
    foreach($siswa as $hsiswa){
      ...
      $objset->setCellValue('B'.$m,'   '.$hsiswa['nisn']);
      $objset->setCellValue('C'.$m,'   '.$hsiswa['nama']);
      if($hsiswa['nilaiuas']==NULL){}else{
        if($hsiswa['nilaiuas']>=$hmapel['sl']){
    ...
    require '../view/rumus/nilaiexpermapel.php';
      ...
      if($nilasli==0){}else{$objset->setCellValue('D'.$m,koma($nilasli));}
      if($nilakhir==0){}else{
        if($nilakhir>=$hmapel['sl']){
      ...
      if($nilakhir==0){}else{$objset->setCellValue('E'.$m,$nilakhir);}
    ...
    foreach($siswa as $hsiswa){
      ...
      $sr= 'D';
      foreach($aspekket as $haspekket){
        $nilaiket=mysqli_fetch_array(mysqli_query($con,"SELECT nilai FROM nilaiket where c_ta='$c_ta' and c_aspekket='$haspekket[c_aspekket]' and c_siswa='$hsiswa[c_siswa]' and c_kelas='$_GET[r]' and c_mapel='$_GET[q]' "));
      ...
      $objPHPExcel->getActiveSheet(5)->getStyle('D'.$m.':'.$sr.$jm)->applyFromArray($header)->getFont()->setSize(10);
      $m++; $no++;
    }
    $objset = $objPHPExcel->setActiveSheetIndex(0);
