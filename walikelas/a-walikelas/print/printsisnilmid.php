...
$content.='
<title>Nilai MID Kelas '.$hkelas['kelas'].'</title>
<h3 class="text-center">Nilai MID '.$hsiswa['nama'].' '.$ata['tahun'].' Semester '.$ata['semester'].'</h3>
<table border="1" cellpadding="2" style="font-size:12px; width: 100%;">
  <tr class="text-center">
    <td width="5%">NO</td><td>MATA PELAJARAN</td><td>KKM</td><td>NILAI MID</td>
  </tr>';
  $ex=mysqli_query($con,"SELECT * FROM kategori_mapel order by posisi asc ");
  while($hex=mysqli_fetch_array($ex)){ 
  $mapel= mysqli_query($con,"SELECT *,(SELECT nilai FROM nilaimid where c_ta='$c_ta' and c_siswa='$_GET[q]' and c_kelas='$_GET[r]' and c_mapel=mapel_kelas.c_mapel) as nilaimid,(select mapel from mapel where c_mapel=mapel_kelas.c_mapel) as mapel,(select mapel from mapel where c_mapel=mapel_kelas.c_mapel) as mapel,(select sl from mapel where c_mapel=mapel_kelas.c_mapel) as sl FROM mapel_kelas where c_kelas='$_GET[r]' and c_katmapel='$hex[c_katmapel]' order by no asc ");
    if(mysqli_num_rows($mapel)>0){
      ...
      $content.= '<td width="15%" class="text-center">'.$hmapel['nilaimid'].'</td>';
      $content.='</tr>';
      $no++; }
    }
  }
