          <div class="box box-info">
            ...
            <h3 class="box-title"> <i class="glyphicon glyphicon-print"></i> Print Rapot Diniyah-Tahfidz <?php echo $kel['kelas']; ?></h3>
            </div>
            <!-- /.box-header -->
...
<?php $smk=mysqli_query($con,"SELECT * FROM siswa where c_kelas='$kel[c_kelas]' order by nama asc "); $vr=1;while($akh=mysqli_fetch_array($smk)){ ?>                
                <tr>
                  ...
                  <td>
                    <a target="_blank" href="<?php echo $basewa; ?>a-walikelas/<?php echo md5('printrapotdintah').'/'.$akh['c_siswa'].'/'.$kel['c_kelas']; ?>" class="btn btn-sm btn-success">Print</a>
                  </td>
                </tr>
                <?php $vr++; } ?>
              </tbody>
