...
$mapel= mysqli_query($con,"SELECT * FROM mapel order by mapel asc "); 
$aspektug= mysqli_query($con,"SELECT * FROM aspektug order by c_aspektug asc "); $jtug= mysqli_num_rows($aspektug);
$content.='
<title>Nilai Tugas Kelas '.$hkelas['kelas'].'</title>
<h3 class="text-center">Nilai Tugas '.$hsiswa['nama'].' '.$ata['tahun'].' Semester '.$ata['semester'].'</h3>
<table border="1" cellpadding="2" style="font-size:12px; width: 100%;">
  <tr class="text-center">
    <td rowspan="2" width="5%">NO</td><td rowspan="2">MATA PELAJARAN</td><td rowspan="2">KKM</td><td colspan="'.$jtug.'">NILAI TUGAS</td>
  </tr>';
  $content.='
  <tr>';
    $ko=1; foreach($aspektug as $haspektug){
  ...
  while($hex=mysqli_fetch_array($ex)){ $colkat =$jtug+2;
    $mapel= mysqli_query($con,"SELECT *,(select mapel from mapel where c_mapel=mapel_kelas.c_mapel) as mapel,(select mapel from mapel where c_mapel=mapel_kelas.c_mapel) as mapel,(select sl from mapel where c_mapel=mapel_kelas.c_mapel) as sl FROM mapel_kelas where c_kelas='$_GET[r]' and c_katmapel='$hex[c_katmapel]' order by no asc "); 
      ...
      foreach($aspektug as $haspektug){
          ...
          $content.= '<td width="5%" class="text-center">'.$nilaitug['nilai'].'</td>';
      }
      $content.='</tr>';
      $no++; }
    }
