  ...
  <div class="col-xs-12 col-md-12 col-lg-12">
    <div class="box box-info">
      ...
      <div class="box-body table-responsive">
              ...
              <tbody>
<?php 
  $pel=mysqli_query($con,"SELECT *,(SELECT count(nilai) FROM nilaiuh where c_ta='$c_ta' and c_siswa='$siswa[c_siswa]' and c_kelas='$siswa[c_kelas]' and c_mapel=mapel_kelas.c_mapel) as nilaiuh,(SELECT count(nilai) FROM nilaitugas where c_ta='$c_ta' and c_siswa='$siswa[c_siswa]' and c_kelas='$siswa[c_kelas]' and c_mapel=mapel_kelas.c_mapel) as nilaitugas,(SELECT nilai FROM nilaimid where c_ta='$c_ta' and c_siswa='$siswa[c_siswa]' and c_kelas='$siswa[c_kelas]' and c_mapel=mapel_kelas.c_mapel) as nilaimid,(SELECT nilai FROM nilaiuas where c_ta='$c_ta' and c_siswa='$siswa[c_siswa]' and c_kelas='$siswa[c_kelas]' and c_mapel=mapel_kelas.c_mapel) as nilaiuas,(SELECT count(nilai) FROM nilaiket where c_ta='$c_ta' and c_siswa='$siswa[c_siswa]' and c_kelas='$siswa[c_kelas]' and c_mapel=mapel_kelas.c_mapel) as nilaiket,(select mapel from mapel where c_mapel=mapel_kelas.c_mapel) as mapel,(select sl from mapel where c_mapel=mapel_kelas.c_mapel) as sl FROM mapel_kelas where c_kelas='$siswa[c_kelas]' and c_katmapel='$hex[c_katmapel]' order by no asc "); $no=1;
    foreach($pel as $hpel){
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <table class="table table-bordered">
            ...
            <tbody>
              <?php $aspektug= mysqli_query($con,"SELECT * FROM aspektug order by c_aspektug asc"); foreach($aspektug as $haspektug){ $amniltug=mysqli_fetch_array(mysqli_query($con,"SELECT nilai FROM nilaitugas where c_ta='$c_ta' and c_aspektug='$haspektug[c_aspektug]' and c_siswa='$siswa[c_siswa]' and c_kelas='$siswa[c_kelas]' and c_mapel='$hpel[c_mapel]' ")); if($amniltug==NULL){} else{ ?>
                <tr>
...
<?php } ?>
    ...
    <table class="table table-bordered">
      ...
      <?php 
      $md=mysqli_query($con,"SELECT * FROM mapelmd_kelas left join mapelmd on (mapelmd.id_mapelmd=mapelmd_kelas.id_mapelmd) where c_kelas='$siswa[c_kelas]' order by nama_mapelmd asc "); $vr=1;
      while($hmd=mysqli_fetch_array($md)){
        $nilmd= mysqli_fetch_array(mysqli_query($con,"SELECT * FROM nilaimd where c_ta='$c_ta' and c_siswa='$siswa[c_siswa]' and c_kelas='$siswa[c_kelas]' and id_mapelmd='$hmd[id_mapelmd]' "));
          echo '
          <tr>
    ...
    <table class="table table-bordered">
      ...
      <?php 
      $md=mysqli_query($con,"SELECT * FROM mapeltq_kelas left join mapeltq on (mapeltq.id_mapeltq=mapeltq_kelas.id_mapeltq) where c_kelas='$siswa[c_kelas]' order by nama_mapeltq asc "); $vr=1;
      while($hmd=mysqli_fetch_array($md)){
        $niltq= mysqli_fetch_array(mysqli_query($con,"SELECT * FROM nilaitq where c_ta='$c_ta' and c_siswa='$siswa[c_siswa]' and c_kelas='$siswa[c_kelas]' and id_mapeltq='$hmd[id_mapeltq]' "));
          echo '
          <tr>
            <td class="text-center">'.$vr.'</td>
